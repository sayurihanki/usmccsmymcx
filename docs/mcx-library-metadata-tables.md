# MCX Library Metadata Tables

Use these tables for the DA block library entry docs at `/.da/library/blocks/<block-name>`.

Do not paste the block authoring rows here. These library docs should contain `library metadata`, not the block content table itself.
